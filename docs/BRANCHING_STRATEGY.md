...
The baseline identification scheme is the tag naming convention above, applied at each milestone. A baseline tag is the audit record that freezes a specific commit as the project's state at a milestone. The scheme encodes three facts in the tag name:

1. **Phase** — `elaboration`, `construction`, or `transition`. This is the RUP phase whose milestone the baseline freezes.
2. **Iteration number** — `E1`, `C2`, `T1`, etc. This is the iteration whose close produced the baseline.
3. **Patch version** — `v1`, `v2`, … This distinguishes a re-tag after a rollback or post-baseline critical fix from the original tag.
